...
model_name=int(input("Which model you would like to train(TYPE THE NUMBER ONLY LIKE 1,2,3)? 1. Densenet 201    2. SE Inception v3   3. Resnet 50"))
import glob
import pickle
import tensorflow as tf
import argparse
...
if __name__ == '__main__':  #straight away go to this
    ...
    elif model_name==3: #IT WILL RUN RESNET 50 MODEL
        Model = Sequential()

        pretrained_model= tf.keras.applications.ResNet50(include_top=False,   #insitializing our modl
                           input_shape=(224,224,3),
                           classes=3,
                           weights='imagenet')
        for layer in pretrained_model.layers:
    ...
    #inception_pred.compile(optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0002), loss=tf.keras.losses.CategoricalCrossentropy(from_logits = False) , metrics=['accuracy'])
        ...
        history=Model.fit(train_data,train_full_label,epochs=100,validation_data=(test_data, val_full_label),callbacks=[early_stopping],batch_size=32)
        my_plots(history,"ResNet50")
        filename = 'ResNet50_model.sav'
        np.save('ResNet50_model.npy',history.history)
        pickle.dump(Model, open(filename, 'wb'))
