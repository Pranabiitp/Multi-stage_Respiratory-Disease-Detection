...
model_name=int(input("Which model you would like to train(TYPE THE NUMBER ONLY LIKE 1,2,3)? 1. Densenet 201    2. SE Inception v3   3. RESNET50"))
import glob
import pickle
import tensorflow as tf
import argparse
...
if __name__ == '__main__':
    normal_dir = "/home/pranab_2021cs25/S_Sharma/pakdd_CONFERENCE/data/normal/" #give your normal cases data path here
    ...
    normal_dir = "/home/pranab_2021cs25/S_Sharma/pakdd_CONFERENCE/data/covid/"  #give your covid 19 cases data path here
    dir1 = os.path.join(normal_dir,"*.npy")
    dir = os.path.join(normal_dir,"*.jpg")
    dir2 = os.path.join(normal_dir,"*.jpeg")
    ...
    normal_dir = "/home/pranab_2021cs25/S_Sharma/pakdd_CONFERENCE/data/pneumonia/" #give your pneumonia cases data path here
    dir1 = os.path.join(normal_dir,"*.npy")
    dir2 = os.path.join(normal_dir,"*.jpeg")
    dir = os.path.join(normal_dir,"*.jpg")
    ...
    #inception_pred.compile(optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0002), loss=tf.keras.losses.CategoricalCrossentropy(from_logits = False) , metrics=['accuracy'])
        ...
        history=Model.fit(train_data,train_full_label,epochs=100,validation_data=(test_data, val_full_label),callbacks=[early_stopping],batch_size=30)
        ...
        early_stopping = EarlyStopping(monitor='val_loss', 
            ...
            min_delta=0.001, 
            mode='min')
        history=Model.fit(train_full_data,train_full_label,epochs=100,validation_data=(val_full_data, val_full_label),callbacks=[early_stopping],batch_size=25) #FITTING THE MODEL
        ...
        pretrained_model= ResNet50(include_top=False,   #insitializing our modl
                           input_shape=(224,224,3),
                           classes=3,
                           weights='imagenet')
        for layer in pretrained_model.layers:
    ...
    #inception_pred.compile(optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.0002), loss=tf.keras.losses.CategoricalCrossentropy(from_logits = False) , metrics=['accuracy'])
        ...
        history=Model.fit(train_data,train_full_label,epochs=100,validation_data=(test_data, val_full_label),callbacks=[early_stopping],batch_size=20)
        my_plots(history,"ResNet50_with_segmentation")
        filename = 'ResNet50_model_with_segmentation.sav'
        np.save('ResNet50_model_with_segmentation.npy',history.history)
        pickle.dump(Model, open(filename, 'wb'))
    else:
